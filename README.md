...
│   ├── riemann_methods.py   # Left/Right/Midpoint classes
│   ├── parser.py            # Function parsing (optional)
│   └── plotter.py           # (Future) Plotting support
│
├── examples/
...
Planned Classes and Components

1-AbstractIntegrator
a-)Defines interface for numerical integration methods
b-)Methods: compute(f, a, b, n)

2-LeftRiemannIntegrator, RightRiemannIntegrator, MidpointRiemannIntegrator
a-)Implement specific formulas for each Riemann method

IntegralEngine
a-)Main class that ties methods together
b-)Provides unified interface:
engine.integrate(f, a, b, n, method="left")

Plotter (Iteration 2)
a-)Visualizes the curve and partition rectangles

Example Usage:
from integrator import IntegralEngine
